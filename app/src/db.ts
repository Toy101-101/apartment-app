...
export interface Schedule extends BaseRow {
  ...
  memo?: string
  /**
   * 済ませた日。**1回きりの予定（`everyMonths: 0`）を済ませたときだけ**入る。
   *
   * 1回きりの予定は、済ませると一覧から消える（`deletedAt` が入る）。
   * だが利用者が消したわけではないので、「消したものを戻す」には出さない。
   * 印が無いと、済んだ予定が「消したもの」として並び、戻すと過ぎた日付のまま
   * ⑤にもどってきて、いつまでも期限切れとして居座る（`lib/trash.ts`）。
   *
   * 索引ではないので、この欄のために版を上げる必要はない。
   */
  completedOn?: string
}

...
### まず、実機での確認（作った側ではなく祖父が触る）

- 幅375px・**端末の文字サイズを最大**にして、横スクロールが出ず全ボタンが押せるか
...
`PLAN.md`「後で足す（v2以降）」を見ること。とくに次の2つを推す。

1. **確定申告用の年間集計** — `expenses` と `payments` はもう揃っているので、足し算だけで作れる
2. **この建物のあゆみ（年表）** — 修繕・更新・家賃改定・入退去を1本の縦の年表にする。
   記録が積み上がって見えること自体が、書体や色よりも強く「アーカイブ」を表現する。
   大家Oneにはない切り口で、このアプリの目的にいちばん近い

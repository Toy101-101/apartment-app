...
- **契約更新の知らせ（60日前・30日前）はいまも固定**。`lib/rent.ts` の `renewalLevel` に
  数字が直接書いてある。⑤年間の予定は予定ごとに選べるので、同じ作りを契約側にも足せる。
  ユーザーから2026-08-11に「後から変えられるか」と質問あり → 「できる」と回答済み（未着手）
...
- 次の候補は**更新案内の日数を選べるようにする**（ユーザーから質問あり）／退去立会いと敷金精算
